...
for (var i=0;i<consolidatedDates.length; i++){
  ...
  if(dateSalePair[dateAti]> largestSaleData['largestSaleDate']){
...
//consolidate the totalDates for each candy to get the quanitity sold per candy
// var consolidatedDatesPerCandy = {name: numerical value}
// pull candy names as an array
// for (master)loop iterate over each candy
// store2['sale date'][candy name] => outputs the date array
//for (nested)loop and return an object where key= name, value = quanitity

//access the price of each candy => store2['inventory prices']

//initialize new k/v pair within store2 with k= revenue v= nested obj of k(candy names)/ v(revenue)
//for loop: map over each object set (sale vs inv price) by candy name
// pull out price and quantity to multiply
//multiply that total by the price
// for each candy add to store2[revenue]
